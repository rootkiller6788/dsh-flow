...
const LEGACY_CARD_POSITIONS_KEY = 'dsh-flow:map-card-positions'
const CARD_POSITIONS_KEY = 'dsh-flow:map-card-positions:v3'
...
const savedBranchAnchors = (() => {
  try {
    const value = JSON.parse(localStorage.getItem('dsh-flow:map-branch-anchors') ?? '[]')
...
const savedCardPositions = (() => {
  try {
    ...
    localStorage.removeItem('dsh-flow:map-card-positions:v2')
...
const state = {
  summaries: [], workspace: null, activeId: null, selectedCardId: null, zoom: 1, currentDsh: null,
...
function rememberBranchAnchor(sessionId, cardId) {
  ...
  try { localStorage.setItem('dsh-flow:map-branch-anchors', JSON.stringify([...state.branchAnchors])) } catch { /* Private browsing may disable local storage. */ }
}
...
function conversationCard(card, graph) {
  ...
  return `<article class="thread-card ${selected}" data-card-id="${escapeHtml(card.id)}" data-position-key="${escapeHtml(card.positionKey)}" data-thread="${card.dshThreadId}" style="left:${card.position.x}px;top:${card.position.y}px;--thread-color:#3478f6">
    ...
    <div class="thread-card-head"><span class="topic-dot"></span><button class="thread-title" data-action="open-dsh" data-thread="${card.dshThreadId}" data-seq="${Number.isInteger(card.sourceSeq) ? card.sourceSeq : ''}" title="在 DSH 中打开完整会话：${escapeHtml(card.question)}">${escapeHtml(card.question)}</button></div>
...
function syncCanvasViewport() {
  if (state.canvasCards === undefined) return
...
function renderCanvas() {
  ...
  if (threads.length === 0 && state.draft?.kind !== 'new') return `<section class="empty-canvas"><strong>当前工作目录还没有 DSH 对话。</strong><p>点击新建会话，在画布中输入第一条消息。</p><div><button class="primary" type="button" data-action="create-session">新建会话</button></div></section>`
...
function render() {
  // Remember the departing inspector's scroll offset per card. There is no
  // other view to remember a position for: the conversation a card points at
  ...
  const inspectorScrollTop = state.inspectorCardId !== null ? state.inspectorScrollByCard.get(state.inspectorCardId) ?? null : null
  ...
  // No 新建 button: creating a session is the host's job (its own sidebar has
  // one). The empty canvas keeps its one 新建会话 button in the stage instead.
  // The row itself is copied from the agent canvas verbatim — the same three
  // controls, the same order, the same text glyphs, no icons, and always
  // rendered, exactly like app.js — because the two rows sit in the same place
  // and any difference between them reads as a bug. 重置 is the only way back to
  // the auto layout: it drops the dragged positions as well as the camera, so
  // 整理 and 定位 are covered by it.
  // 转换 sits leftmost in both layers' control rows — it leaves this layer,
  // every other control acts on it — so the two rows stay word-for-word
  // identical and either layer can hand over to the other.
  const canvasControls = `<div class="canvas-controls"><button type="button" data-action="switch-layer" title="转换到团队图层：同一张画布的另一层">转换</button><button type="button" data-action="reset-canvas" title="重置画布：恢复自动布局与默认视角">重置</button><button type="button" data-action="zoom-out" aria-label="缩小" title="缩小">−</button><span class="zoom-label">${Math.round(state.zoom * 100)}%</span><button type="button" data-action="zoom-in" aria-label="放大" title="放大">＋</button></div>`
  app.innerHTML =`<main class="synapse-shell"><header class="topbar">${canvasControls}</header><section class="main-stage">${state.error ? `<div class="status-message" role="alert"><span>${escapeHtml(state.error)}</span><button data-action="dismiss-error" aria-label="关闭" title="关闭">×</button></div>` : ''}${renderCanvas()}${selectionFollowupButton()}</section></main>`
  installDragging()
  cacheCardConnectors()
...
function selectionFollowupTarget(range) {
  ...
  // Only card answers can be followed up from here: the map has no conversation
  // page of its own, so a selection inside one is impossible.
...
document.addEventListener('keydown', event => {
  if (event.key !== 'Escape' || state.inspectorCardId === null) return
  event.preventDefault()
...
app.addEventListener('click', async event => {
  ...
  if (!(button instanceof HTMLElement)) {
    ...
    // without closing the map; the client confirms via flow:current-session.
  ...
  try {
    ...
    // The map draws the graph; the conversation itself belongs to the host, so
    // every "open this session" affordance is a jump to the native Chat tab.
    ...
    if (button.dataset.action === 'toggle-message' && button.dataset.message !== undefined) { state.expandedMessageIds.has(button.dataset.message) ? state.expandedMessageIds.delete(button.dataset.message) : state.expandedMessageIds.add(button.dataset.message); render() }
    ...
    // 转换 hands the frame back to the team layer; the host client owns the
    // swap, since both layers live in one iframe it created.
    if (button.dataset.action === 'switch-layer') post('flow:switch-layer')
    if (button.dataset.action === 'dismiss-error') { state.error = ''; render() }
    // 重置 puts the canvas back to its starting state: auto layout (forget the
    // dragged positions), the initial camera, and 100%. resetCanvasCamera
    // clears canvasViewInitialized, so the next renderCanvas re-centers on the
    // active card — which is what 定位 used to do on demand.
    if (button.dataset.action === 'reset-canvas' && state.workspace !== null) {
      resetCardPositions()
      resetCanvasCamera()
      state.zoom = 1
      render()
...
app.addEventListener('submit', event => {
  ...
  // The draft card is the only composer on the canvas now: 追问 / 分支 / 新建会话
  // all funnel through submitDraft.
  if (form.matches('[data-draft]')) { event.preventDefault(); void submitDraft() }
})

window.addEventListener('message', event => {
  if (event.origin !== window.location.origin || event.data?.source !== 'dsh-flow') return
  const data = event.data
  if (data.type === 'flow:map-opened') {
    ...
    render()
    window.requestAnimationFrame(() => post('flow:map-ready'))
  }
